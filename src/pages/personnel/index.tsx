...
function PersonnelPage(state: any) {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "200px",
      render: (text: any) => {
        return state.PositionList.map((item: any) => { if (item.id == text) { return item.positionName } })
      }
    },
    {
      title: '邮箱',
        ...
        return (
          <div>
            <a style={{ marginLeft: 10 }} onClick={() => {
              ...
              state.dispatch({ type: 'personnel/Info', payload: { id: row.id } })
            }}>
              编辑
            </a>
            <Popconfirm
              title="确定删除?"
              onConfirm={() => {
                state.dispatch({ type: 'personnel/Delete', payload: { id: row.id } })
              }}
              onCancel={() => { }}
              okText="确定"
              cancelText="取消"
            ...
            </Popconfirm>
            {
              row.isSend == 0 ?
                <a style={{ marginLeft: 10 }} onClick={() => {
                  const list = { name: row.name, sex: row.sex, Email: row.Email, id: row.id }
                  state.dispatch({ type: 'personnel/save', payload: { ShowVisible: true, SendEmail: list } });
                }}> 发送面试邀请 </a> : ""
            }
          </div>
        );
  ...
  function confirm() {
    Addform
      ...
      .then((values) => {
        ...
        state.dispatch({ type: 'personnel/UpDate', payload: { params: List, id: AddParams.id ? AddParams.id : '' } });
      })
      .catch((errorInfo) => {
        console.log(errorInfo);
      });
  ...
  function cancel() {
    state.dispatch({ type: 'personnel/cancel' })
    state.dispatch({ type: 'personnel/save', payload: { Visible: false, Update: true } })
  ...
  function Showconfirm() {
    SendMessage
      .validateFields()
      .then((values) => {
        state.dispatch({ type: 'personnel/SendMessage', payload: { ...state.SendEmail, time: moment(values.time).format('YYYY-MM-DD HH:mm:ss') } })
      })
      .catch((errorInfo) => {
        console.log(errorInfo);
      });
  ...
  function ShowCancel() {
    state.dispatch({ type: 'personnel/save', payload: { ShowVisible: false, SendEmail: {} } });
  }
  ...
  return (
    <div>
            ...
            <Row>
              ...
              <Col md={12} xl={8} lg={8} sm={24}>
                <Form.Item label="学历" name="education" initialValue={state.params.education}>
                  <Select dropdownMatchSelectWidth={false}
                    ...
                    filterOption={false}
                  >
                    <Option value='-1' key='99'>全部</Option>
              ...
              <Col md={12} xl={8} lg={8} sm={24}>
                <Form.Item label="工作状态" name="workStatus" initialValue={state.params.workStatus}>
                  ...
                  >
                    <Option value='-1' key='99'>全部</Option>
                    <Option value='1' key='1'>离校</Option>
                    <Option value='2' key="2">在校</Option>
                    <Option value='3' key='3'>在职</Option>
                    <Option value='4' key='4'>离职</Option>
                  </Select>
                </Form.Item>
              </Col>
              <Col span={24}>
                <Form.Item>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      state.dispatch({ type: 'personnel/save', payload: { params: List } })
                      state.dispatch({ type: 'personnel/save', payload: { Update: true } })
                    }
                    }>
                    重置
                  </Button>
                  <Button style={{ margin: '0 8px' }} onClick={() => {
                    state.dispatch({ type: 'personnel/cancel' })
                    state.dispatch({ type: 'personnel/save', payload: { Visible: true, Update: true } })
                  }}>
                    新增
                  </Button>
                </Form.Item>
        ...
        >
          <Row>
            <Col md={12} xl={12} lg={12} sm={12}>
              <Form.Item label="姓名" name="name" initialValue={AddParams.name}
                rules={[{ required: true, message: '请输入您的姓名' }]}>
                <Input />
              </Form.Item>
            </Col>
            <Col md={12} xl={12} lg={12} sm={12}>
              <Form.Item label="年龄" name="age" initialValue={AddParams.age}
            ...
            <Col md={12} xl={12} lg={12} sm={12}>
              <Form.Item label="性别" name="sex" initialValue={AddParams.sex}>
                <Select dropdownMatchSelectWidth={false}
                  ...
                  filterOption={false}
                >
                  <Option value='1' key={10}>男</Option>
                  <Option value='2' key={20}>女</Option>
                </Select>
              </Form.Item>
            </Col>
            <Col md={12} xl={12} lg={12} sm={12}>
              <Form.Item label="工作状态" name="workStatus" initialValue={AddParams.workStatus}>
                <Select dropdownMatchSelectWidth={false}
                  ...
                  filterOption={false}
                >
                  <Option value='0' key='0'>离校</Option>
                  <Option value='1' key="1">在校</Option>
                  <Option value='2' key='2'>在职</Option>
                  <Option value='3' key='3'>离职</Option>
                </Select>
              </Form.Item>
            </Col>
            <Col md={12} xl={12} lg={12} sm={12}>
            ...
            <Col md={12} xl={12} lg={12} sm={12}>
              <Form.Item label="学历" name="education" initialValue={AddParams.education}
            ...
            <Col md={12} xl={12} lg={12} sm={12}>
              <Form.Item label="邮箱" name="Email" initialValue={AddParams.Email}
                rules={[{ required: true, message: '请输入您的邮箱' }]}>
                <Input />
              </Form.Item>
            </Col>
            <Col md={12} xl={12} lg={12} sm={12}>
              <Form.Item label="联系方式" name="phone" initialValue={AddParams.phone}
      ...
      <Modal
        ...
        width="600px"
        footer={[
          <Button type="primary" onClick={Showconfirm}>发送</Button>,
